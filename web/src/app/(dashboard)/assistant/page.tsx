...
import { Users, CreditCard, Search, CheckCircle2, Clock, Receipt } from "lucide-react";
import { mockAppointments, mockInvoices } from "@/lib/mock-data";
import type { AppointmentStatus, InvoiceStatus } from "@/lib/types";
import { appointmentService, type Appointment } from "@/app/services/appointment.service";
...
export default function AssistantPage() {
        ...
        async function loadData() {
            try {
                ...
                if (profile.clinicId) {
                    ...
                    setQueue(allAppointments.filter(a => a.status === "CONFIRMED" || a.status === "PENDING"));
    ...
    const filteredInvoices = useMemo(() => {
        ...
        return pendingInvoices.filter((i: any) =>
            i.id.toLowerCase().includes(invoiceSearch.toLowerCase()) ||
            i.owner?.firstName?.toLowerCase().includes(invoiceSearch.toLowerCase()) ||
            ...
            <div className="space-y-8 p-4 max-w-7xl mx-auto">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">Manage check-ins, queue, and process billing.</p>
                    </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-blue-600">{inConsultCount}</div>
                            <p className="text-xs text-muted-foreground">Currently with a vet</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-red-600">{pendingInvoices.length}</div>
                            <p className="text-xs text-muted-foreground">
                                Rs. {pendingInvoices.reduce((s, i) => s + i.amount, 0).toLocaleString()} outstanding
                            </p>
                        </CardContent>
                    </Card>
                </div>
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {filteredQueue.filter(q => q.status !== "COMPLETED").length === 0 ? (
                                        <TableRow>
                                            <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No patients in queue.</TableCell>
                                        </TableRow>
                                    ) : filteredQueue.filter(q => q.status !== "COMPLETED").map((item) => (
                                        <TableRow key={item.id}>
                                            <TableCell className="font-medium">
                                                {item.pet?.name}
                                                <span className="text-xs text-muted-foreground ml-1">({item.pet?.species})</span>
                                            </TableCell>
                                            <TableCell className="text-muted-foreground">{item.owner?.firstName} {item.owner?.lastName}</TableCell>
                                            <TableCell className="text-sm">
                                                {new Date(item.date).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}
                                            </TableCell>
                                            <TableCell>
                                                {arrivedAppointments.has(item.id) ? (
                                                    <Badge className="bg-green-500 hover:bg-green-600">Arrived</Badge>
                                                ) : (
                                                    <Badge
                                                        variant={item.status === "CONFIRMED" ? "default" : "secondary"}
                                                        className={item.status === "CONFIRMED" ? "bg-blue-500 hover:bg-blue-600" : "bg-amber-100 text-amber-800"}
                                                    >
                                                        {item.status === "CONFIRMED" ? "In Consult" : "Waiting"}
                                                    </Badge>
                                                )}
                                            </TableCell>
                                            <TableCell className="text-right">
                                                {item.status === "PENDING" ? (
                                                    <Button size="sm" onClick={() => handleCheckIn(item.id)}>Check In</Button>
                                                ) : item.status === "CONFIRMED" ? (
                                                    arrivedAppointments.has(item.id) ? (
                                                        <span className="text-sm text-muted-foreground">Arrived</span>
                                                    ) : (
                                                        <div className="flex justify-end gap-2">
                                                            <Button size="sm" variant="outline" onClick={() => handleArrived(item.id)}>
                                                                Arrived
                                                            </Button>
                                                            <Button size="sm" variant="destructive" onClick={() => handleNoShow(item.id)}>
                                                                Not Show
                                                            </Button>
                                                        </div>
                                                    )
                    ...
                    <div className="space-y-4">
                        <Card className="bg-zinc-900 text-zinc-50 border-zinc-800">
                            <CardHeader>
                                ...
                                <CardDescription className="text-zinc-400">Process pending payments.</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-3">
                                ...
                                <div className="space-y-2 max-h-[300px] overflow-y-auto">
                                    {filteredInvoices.length === 0 ? (
                                        <p className="text-zinc-500 text-sm text-center py-4">No pending invoices.</p>
                                    ) : filteredInvoices.map(inv => (
                                        <div key={inv.id} className="flex items-center justify-between p-3 rounded-lg bg-zinc-800 border border-zinc-700">
                                            <div>
                                                <p className="font-mono text-xs text-zinc-400">{inv.id.toUpperCase()}</p>
                                                <p className="text-sm font-medium">{(inv as any).owner?.firstName} {(inv as any).owner?.lastName}</p>
                                                <p className="text-lg font-bold text-amber-400">Rs. {inv.amount.toLocaleString()}</p>
                                            </div>
                                            <Button
                                                size="sm"
                                                variant="secondary"

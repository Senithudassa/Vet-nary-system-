...
import { Search, Stethoscope, FileText, Syringe } from "lucide-react";
        ...
        async function loadPetData() {
            ...
            try {
                const [petTimeline, allAppointments] = await Promise.all([
                    ...
                    clinicId ? appointmentService.getClinicAppointments(clinicId) : Promise.resolve([])
                ]);
                setTimeline(petTimeline);
                setAppointments(allAppointments.filter((a: any) => a.petId === selectedPetId));
            } catch (error) {
        ...
        <ProtectedRoute allowedRoles={["vet", "main_admin"]}>
            <div className="space-y-6 p-4 max-w-7xl mx-auto">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">View medical history, vaccinations, appointments and VetBook timeline for any allocated pet.</p>
                </div>
                ...
                <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
                    ...
                    <Card className="h-fit">
                        ...
                        <CardContent className="space-y-1 max-h-[400px] overflow-y-auto">
                            {isLoading ? (
                                <p className="text-sm text-muted-foreground text-center py-4">Loading pets...</p>
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="text-xs text-muted-foreground mt-1">
                                                    Owner: {selectedPet?.owner?.firstName} {selectedPet?.owner?.lastName} {selectedPet?.owner?.phone && `(${selectedPet.owner.phone})`}
                                                        ...
                                                        <Card>
                                                            <CardHeader className="pb-2">
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <Badge variant="outline" className="ml-auto text-xs">
                                                                        {new Date(item.recordDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                                                                    </Badge>
                                                                </div>
                                                            </CardHeader>
                                                            <CardContent className="text-sm text-muted-foreground space-y-1">
                                                                {item.type === "MEDICAL" ? (
                                                                    <>
                                                                        {item.treatment && (
                                                                            <p><span className="font-medium text-foreground">Treatment:</span> {item.treatment}</p>
                                                                        )}
                                                                        {item.prescription && (
                                                                            <p><span className="font-medium text-foreground">Prescription:</span> {item.prescription}</p>
                                                                        )}
                                                                        {item.notes && (
                                                                            <p><span className="font-medium text-foreground">Notes:</span> {item.notes}</p>
                                                                        )}
                                                                        <p className="text-xs">By Dr. {item.vet?.firstName} {item.vet?.lastName} at {item.clinic?.name}</p>
                                                                    </>
                                                                ) : (
                                                                    <>
                                                                        <p><span className="font-medium text-foreground">Batch:</span> {item.batchNumber}</p>
                                                                        {item.nextDueDate && (
                                                                            <p><span className="font-medium text-foreground">Next Due:</span> {new Date(item.nextDueDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p>
                                                                        )}
                                                                        <p className="text-xs">By Dr. {item.administeredBy?.firstName} {item.administeredBy?.lastName} at {item.clinic?.name}</p>
                                                                    </>
                                                                )}
                                                            </CardContent>
                                                        </Card>
                                                ...
                                                <Table>
                                                    ...
                                                    <TableBody>
                                                        {appointments.length === 0 ? (
                                                            ...
                                                            <TableRow key={apt.id}>
                                                                <TableCell className="text-sm">{new Date(apt.date).toLocaleString([], { dateStyle: "short", timeStyle: "short" })}</TableCell>
                                                                <TableCell className="font-medium">{apt.reason || "—"}</TableCell>
                                                                <TableCell>
                                                                    <Badge variant={apt.status === "COMPLETED" ? "default" : apt.status === "PENDING" ? "secondary" : apt.status === "CANCELLED" ? "destructive" : "outline"}>
                                                                        {apt.status}
                                                                    </Badge>
                                                                </TableCell>
                                                                <TableCell className="text-sm">{apt.vet ? `Dr. ${apt.vet.firstName} ${apt.vet.lastName}` : "—"}</TableCell>
                                                            </TableRow>
                                                        ))}
                                                    </TableBody>
                                                </Table>

...
import { mockClinics, mockSupportTickets } from "@/lib/mock-data";
...
export default function MinorAdminPage() {
  ...
  const [stagedChanges, setStagedChanges] = useState([
    {
      desc: "Increase Platform Discount for CL-01 to 20%",
      author: "Admin Sarah",
      time: "10:15 AM",
      status: "Awaiting Main Admin",
    },
    {
      desc: "Reset VN-3310 Password",
      author: "Admin Mike",
      time: "09:45 AM",
      status: "Approved, Queued for 12:00 PM",
    },
  ]);
  ...
  const handleProposeChange = () => {
    ...
    toast.success(
      "Configuration change proposed. Awaiting Main Admin approval.",
    );
  };
      ...
      <div className="space-y-8 p-4 max-w-7xl mx-auto">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">
              Contact Center (Minor Admin)
            </h1>
            <p className="text-muted-foreground mt-1">
              Resolve customer tickets, evaluate clinic tiers, and propose
              system changes.
            </p>
          </div>
          <div className="flex gap-2 text-sm">
            <Button
              variant="outline"
              onClick={() =>
                toast.info("Status: Active and available for incoming tickets.")
              }
            >
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card className="bg-primary text-primary-foreground">
            ...
            <CardContent>
              ...
              <p className="text-xs text-primary-foreground/80 mt-1">
                {supportTickets.filter((t) => t.status === "OPEN").length} need
                attention
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {stagedChanges.length} Proposed
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                System pushes at exact 12:00 PM
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="border-primary/20 shadow-sm">
          <CardHeader className="bg-primary/5 pb-4 border-b">
            ...
            <CardDescription>
              Address customer issues. If it requires clinic intervention, route
              the ticket directly to the Vet Branch.
            </CardDescription>
          </CardHeader>
        ...
        <div className="grid gap-8 grid-cols-1 lg:grid-cols-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Assess clinic volume against hashed target metrics to evaluate
                  ...
                  {clinicTiers.map((tier) => (
                    <TableRow key={tier.id}>
                      ...
                      <TableCell className="text-right">
                        <Badge
                          variant={
                            tier.discountTier.includes("Gold")
                              ? "default"
                              : tier.discountTier.includes("Silver")
                                ? "secondary"
                                : "outline"
                          }
                        >
          ...
          <Card className="border-amber-500/30">
            <CardHeader className="bg-amber-500/5 border-b border-amber-500/10">
              <CardTitle className="text-amber-800 dark:text-amber-500 flex items-center gap-2">
                <GitCommitHorizontal className="h-5 w-5" /> Pending Commits
                (Staging)
              </CardTitle>
              <CardDescription>
                Changes you make to system configurations require Main Admin
                approval. All approved edits go live for Vets at 12:00 PM.
              </CardDescription>
            </CardHeader>
                ...
                {stagedChanges.map((change, idx) => (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span
                        className={`font-semibold flex items-center gap-1 ${change.status.includes("Approved") ? "text-green-600" : "text-amber-600"}`}
                      >
                        {change.status.includes("Approved") && (
